...
from ha_testcontainer.visual.cursors import CURSOR_SVGS as _CURSOR_SVGS
...
# Set this to your project's scenarios directory before running tests, e.g.::
#
...
# When None, _assert_snapshot_with_threshold uses the calling test file's
# directory (the same behaviour as assert_snapshot).
SNAPSHOTS_DIR: Path | None = None

# Repository root — doc image ``output`` paths are resolved relative to here.
# Defaults to the current working directory (the pytest invocation directory).
...
def load_all_scenarios() -> list[dict[str, Any]]:
    ...
    Documentation-only scenarios living under ``docs/scenarios/`` are
    intentionally excluded so that ``test_scenarios.py`` does not run them as
    functional tests.

    :data:`SCENARIOS_DIR` must be set before calling this function.  Set it
    ...
    if SCENARIOS_DIR is None:
        raise RuntimeError(
            "ha_testcontainer.visual.scenario_runner.SCENARIOS_DIR is not configured. "
            "Set it to the path of your scenarios directory before running tests, e.g. "
            "in your conftest.py:\n"
            "  import ha_testcontainer.visual.scenario_runner as sr\n"
            "  sr.SCENARIOS_DIR = Path(__file__).parent / 'scenarios'"
        )
...
def _assert_snapshot_with_threshold(
    ...
    *,
    ...
    if _snapshots_dir is None:
        ...
        for frame_info in _inspect.stack()[2:]:
            caller_file = Path(frame_info.filename).resolve()
            if caller_file != this_file:
                _snapshots_dir = caller_file.parent / "snapshots"
                break
        else:
            _snapshots_dir = Path.cwd() / "snapshots"
    _snapshots_dir.mkdir(parents=True, exist_ok=True)
    baseline = _snapshots_dir / f"{name}.png"
    actual = _snapshots_dir / f"{name}.actual.png"
