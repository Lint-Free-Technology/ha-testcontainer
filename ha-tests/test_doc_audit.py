"""Audit test: every PNG/GIF image referenced in the docs must be either
generated by a ``doc_image:`` / ``doc_animation:`` scenario, or explicitly
listed in ``ha-tests/doc-image-audit-exclusions.txt``.
...
.. code-block:: bash
    ...
    pytest ha-tests/test_doc_audit.py
...
Exclusion list
--------------
``ha-tests/doc-image-audit-exclusions.txt`` contains one repo-relative path
per line.  Lines that start with ``#`` and blank lines are ignored.  Add a
path here for any image that is deliberately *not* auto-generated by a
scenario (diagrams, logos, screenshots that cannot or need not be reproduced
in automated tests).

Configuring docs location
--------------------------
By default the test scans the ``docs/source/`` sub-directory at the
repository root for Markdown image references.  Override with the
``DOCS_SOURCE_DIR`` environment variable to point at a different directory.
The test is skipped automatically when the configured directory does not
exist.
"""
...
# Docs directory — configurable via env var; skipped when it doesn't exist.
_docs_source_env = os.environ.get("DOCS_SOURCE_DIR")
DOCS_SOURCE = Path(_docs_source_env) if _docs_source_env else REPO_ROOT / "docs" / "source"

SCENARIO_DIRS = [
    Path(__file__).parent / "visual" / "scenarios",
...
def _collect_scenario_outputs() -> set[str]:
    ...
    Scans all scenario YAML files under ``ha-tests/visual/scenarios/`` and
...
def test_all_doc_images_are_tracked() -> None:
    """Every PNG/GIF referenced in the docs must be scenario-generated or excluded.
    ...
    2. Add the path to ``ha-tests/doc-image-audit-exclusions.txt`` because the
       image is intentionally hand-crafted (diagram, logo, etc.).

    Set ``DOCS_SOURCE_DIR`` to point at your docs directory, or leave unset to
    default to ``docs/source/`` at the repository root.
    """
    if not DOCS_SOURCE.exists():
        pytest.skip(
            f"Docs source directory not found: {DOCS_SOURCE}. "
            "Set DOCS_SOURCE_DIR to enable this test."
        )
