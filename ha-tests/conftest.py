"""Shared pytest fixtures for Lovelace visual tests.

The session-scoped ``ha`` fixture starts one Home Assistant container for the
whole test run, mounts custom components from the local repository, and
optionally sets up a component integration so tests can exercise it against a
real HA instance.
...
HA_URL
    ...
    Start a persistent instance with ``make ha-tests-up`` (or
...
LOVELACE_SETUP_INTEGRATION
    Integration domain to configure via the HA config-flow API immediately
    after the container starts (e.g. ``uix``).  Leave unset or empty to skip
    automatic integration setup — useful when the component registers itself
    automatically or when no custom integration is needed.
"""
...
HA_CONFIG_DIR = Path(__file__).parent / "ha-config"
...
def ha(ha_version: str, tmp_path_factory):
    """Session-scoped HA instance with custom components mounted and configured.

    **Normal mode** (default): starts a fresh Docker container, mounts the
    local ``custom_components/`` directory, optionally sets up an integration
    (controlled by ``LOVELACE_SETUP_INTEGRATION``), and tears everything down
    ...
    **Fast-iteration mode**: when ``HA_URL`` *and* ``HA_TOKEN`` are set in the
    ...
    pytest::
        ...
        # Terminal 1
        make ha-tests-up        # keeps running — Ctrl-C to stop

        # Terminal 2
        source .ha_env
        pytest ha-tests/visual/test_scenarios.py -k my_scenario   # instant!
    ...
    # Mount local custom_components alongside the temp config dir.
