...
Card styling and forge tests live in ``ha-tests/visual/scenarios/`` as YAML files
and are run by ``test_scenarios.py``.

Prerequisites
-------------
These tests are UIX-specific.  Ensure UIX is installed before running::

    make setup COMPONENT=Lint-Free-Technology/uix
    LOVELACE_SETUP_INTEGRATION=uix pytest ha-tests/visual/test_uix_styling.py
"""
