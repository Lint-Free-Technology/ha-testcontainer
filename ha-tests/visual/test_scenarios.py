"""YAML-driven parametrised visual tests.
...
scenario.  Tests are collected automatically and can be run individually::
    ...
    # Run all scenarios
    pytest ha-tests/visual/test_scenarios.py

    # Run a single scenario by its ``id`` field
    pytest ha-tests/visual/test_scenarios.py -k card_basic_style

    # Run all forge scenarios
    pytest ha-tests/visual/test_scenarios.py -k forge

Adding a new test
-----------------
Create a new ``.yaml`` file in ``ha-tests/visual/scenarios/`` (or any
...
def test_scenario(
    ...
    ``lovelace-tests`` Lovelace dashboard, navigates to it, runs all declared
